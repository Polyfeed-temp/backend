...
def create_highlight(db:Session, highlight_data: FeedbackHighlightPydantic):
    ...
    db_highlight= Highlight(id=str(highlight_data.id), url= str(highlight_data.url), startMeta= start_meta, endMeta= end_meta, text= highlight_data.text, annotation_tag= highlight_data.annotation_tag, notes= highlight_data.notes)
